            ...
            override fun onReceive(context: Context, intent: Intent) {
                ...
                if (intent.action==DownloadManager.ACTION_DOWNLOAD_COMPLETE) {
                    Toast.makeText(this@UrlAndDownload, "Download Completed", Toast.LENGTH_SHORT)
                        .show()
                    intent.extras?.let {
                        val fileDownload=File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).toString() +"/" + fileName)
                        val uriForDownloadFile:Uri=FileProvider.getUriForFile(this@UrlAndDownload,"com.main.kotlin"+".fileprovider",fileDownload)
